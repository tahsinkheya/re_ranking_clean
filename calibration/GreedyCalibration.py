...
class GreedyCalibration(object):
    ...
    def compute_diversity_score(self, reco_items, uid, scores, b):
        ...
        reco_dist = self.get_recom_distribution(
            reco_items, uid, compare_dist, alpha
        )  # sum wr(i)q˜(д|i),
    ...
    def get_sensitive_genre_dist(self):
        ...
        tradeoff = 0.01
        self.sensitive_compare_dist = (
            tradeoff * nonzeroarray + (1 - tradeoff) * self.sensitive_compare_dist
        )  # just to ensure we never get log 0 error
        self.sensitive_compare_dist = (
    ...
    def get_new_recommendations(self, reco, scores, all_items):
        """reco is [num of users x k] and scores is  [num of users x num of items]"""
