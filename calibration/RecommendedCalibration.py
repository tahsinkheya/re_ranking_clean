...
class RecommendedCalibration(object):
    ...
    def get_all_user_recommended_genre_dist(self, topk_reco):
        ...
        #     merged_df.groupby("userID")[self.unique_genres].sum().reset_index()
        # )
        genre_prop = summed_genre[self.unique_genres].div(
            summed_genre["user_weight_factor"], axis=0
        )

        summed_genre[self.unique_genres] = genre_prop
    ...
    def compute_diversity_score(self, reco_items, uid, l, scores, b):
        ...
        gender_genre_dist = self.recommended_dist_gender
    ...
    def get_new_recommendations(self, reco, scores, all_items):
        ...
        x = self.get_all_user_recommended_genre_dist(reco)
        recommended_dist_gender = pd.merge(x, self.gender_df, on="userID")
        gender_genre_weights_r = recommended_dist_gender.groupby("Gender")[
            self.unique_genres
        ].mean()
        self.recommended_dist_gender = gender_genre_weights_r.sort_index()
